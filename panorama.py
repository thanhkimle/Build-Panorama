...
def findMatchesBetweenImages(image_1, image_2, num_matches):
    ...
    bfm = cv2.BFMatcher(normType=cv2.NORM_HAMMING, crosCheck=True)
...
def findHomography(image_1_kp, image_2_kp, matches):
    ...
    H, mask = cv2.findHomography(image_1_points, image_2_points, cv2.RANSAC, 5.0)
...
def getBoundingCorners(corners_1, corners_2, homography):
    ...
    xy_min = np.array([min(corners[:, 0, 0]), min(corners[:, 0, 1])], dtype=np.float64)
    xy_max = np.array([max(corners[:, 0, 0]), max(corners[:, 0, 1])], dtype=np.float64)
...
def warpCanvas(image, homography, min_xy, max_xy):
    ...
    H_tran = np.dot(trans_M, homography)

    return cv2.warpPerspective(image, H_tran, canvas_size)
...
def createImageMask(image):
    ...
    mask = cv2.drawContours(thresh, contours, -1, (0, 255, 0), 3)
...
def createRegionMasks(left_mask, right_mask):
    ...
    '''
    # TODO: WRITE YOUR CODE HERE
    left_AND_NOT_right = np.bitwise_and(left_mask, np.logical_not(right_mask))
    left_AND_right = np.bitwise_and(left_mask, right_mask)
    right_AND_NOT_left = np.bitwise_and(right_mask, np.logical_not(left_mask))

    return left_AND_NOT_right, left_AND_right, right_AND_NOT_left
...
def generateAlphaWeights(left_distance,right_distance):
    ...
    '''
    # TODO: WRITE YOUR CODE HERE
    return right_distance/(right_distance + left_distance)
...
def blendImagePair(image_1, image_2, num_matches):
    ...
    **************************************************************************
    ...
    """
    # print("img 1", image_1.shape)
    # print("img 2", image_2.shape)
    ...
    # cv2.imwrite("left_mask.jpg", left_mask.astype(np.uint8)*255)
    right_mask = createImageMask(right_image)
    # cv2.imwrite("right_mask.jpg", right_mask.astype(np.uint8)*255)
    left_AND_right_mask, overlap_region_mask, right_AND_NOT_left_mask = createRegionMasks(left_mask, right_mask)
    left_AND_right_dist_mask = findDistanceToMask(left_AND_right_mask)
    right_AND_NOT_left_dist_mask = findDistanceToMask(right_AND_NOT_left_mask)
    alpha_weight = generateAlphaWeights(left_AND_right_dist_mask, right_AND_NOT_left_dist_mask)
    # print("alpha", alpha_weight.shape)
    # output_image = left_image * alpha_weight + (1-alpha_weight) * right_image
    output_image = np.einsum('ij,ijk->ijk', alpha_weight, left_image) + \
                   np.einsum('ij,ijk->ijk', (1-alpha_weight), right_image)
